{{--@dd($filefields)--}}
{{--@dd($documents)--}}
{{--@dd($document->doc_type)--}}
@if(isset($documents))
    @foreach($documents as $document)
        <label>{{ __('fieldnames.'.$document->doc_type) }}:</label>
        <div class="ex-upload-item tw-flex">
            ...
            <form action="{{ route('delete-document', ['id' => $document->id])  }}" method="POST" class="">
                @csrf
                @method('DELETE')
                <button class="btn btn-success doc-delete-button">x</button>
            </form>
        </div>
    @endforeach
@endif
...
@if(isset($object))
    @foreach($filefields as $filefield)
        ...
        @endif
        @if(isset($object->$fieldname) && is_object($object->$fieldname))
            <div class="ex-upload-item tw-flex">
                ...
                <form action="{{ route('delete-old-document', ['object_type' => $object->child_prefix, 'object_id' => $object->id, 'document_type' => $fieldname])  }}"
                      method="POST">
                    @csrf
                    @method('PUT')
                    <button class="btn btn-success doc-delete-button">x</button>
...
@foreach($filefields as $filefield)
    ...
    <div class="form-group">
        @if($filefield[1] == 'file')
            <div class="label" id="title-filefield-{{$fieldname}}">
                <span style="color: red">*</span> {{ __('fieldnames.'.$fieldname) }}
        ...
        <!--    clone hide        -->
            ...
            <div class="input-group-btn">
                <button id="filefield-{{$fieldname}}" class="btn-success btn-add-doc" data-id="filefield-{{$fieldname}}"
                        type="button"><i class="fldemo glyphicon glyphicon-plus"></i>{{__('add document')}}
                </button>
            </div>
