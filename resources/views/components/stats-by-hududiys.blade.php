                    ...
                    <div class="row justify-content-between">
                        <div class="col-md-8">
                            <h3 class="table-list-title">
                                <i data-feather="box" class="mr-2"></i> {{ __('stats-by-hududiys-title') }}
                            </h3>
                        </div>
                        <div class="col-md-4">
                            <a href="{{ route('ukn.exports.index', \Request::all()) }}" class="btn-label float-right bg-success mr-3 mb-4">
                                Excel
                    ...
                    <form method="GET">
                        <div class="row mb-4">
                            @foreach($filters as $filter)
                                <div class="col-auto">
                                    <select name="{{ $filter['code'] }}" class="ex-select ex-select_default filter-select dropdown form-control">
                                        <option disabled selected>{{ __($filter['name']) }}</option>
                                        @foreach($filter['list'] as $name)
                                            <option @if($filter['current'] == $name) selected @endif value="{{ $name }}">{!! __($name) !!}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @endforeach
...
@endif
                        <table class="table table-default">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="td-w3 text-center"><button class="table-action text-white">{{ __('Hududiy') }}</button></th>
                                <th scope="col"><button class="table-action text-white">{{ __('applications_created') }}</button></th>
                                <th scope="col"><button class="table-action text-white">{{ __('objects_created') }}</button></th>
{{--                                <th scope="col"><button class="table-action text-white">{{ __('objects_deleted') }}</button></th>--}}
                                <th scope="col"><button class="table-action text-white">{{ __('Endpoints') }}</button></th>
                            </tr>
                            </thead>
                            <tbody>
